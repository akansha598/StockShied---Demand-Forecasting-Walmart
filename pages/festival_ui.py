...
@st.cache_data
def fetch_festivals(country, year):
    API_KEY = "OhHyD27RJFZbZsE8YsYgyVkcGUA9BfBG"
    url = f"https://calendarific.com/api/v2/holidays?&api_key={API_KEY}&country={country}&year={year}"
    ...
    try:
        ...
        if "response" not in data or "holidays" not in data["response"]:
            st.error("❌ Calendarific API error — invalid response. Check API key or country.")
            return pd.DataFrame()
        ...
        holidays = [
            ...
            for item in data["response"]["holidays"]
            if item.get("type") and "National holiday" in item["type"]
        ]
...
st.title("🎉 Festival-Based Walmart Sales Forecast")
st.markdown("Predict sales based on upcoming national festivals in your country.")
...
if not festivals_df.empty:
    st.success("✅ Upcoming national festivals loaded!")
    ...
    st.warning("⚠️ No upcoming national festivals found for the selected country or year.")
